...
export function NftCreationModal(){
  ...
  async function changeImageFunc(element){
      ...
      <div className="bg-[#151c25] shadow-xl shadow-[#25bd9c] rounded-xl w-11/12 sm:w-2/5 h-7/12 p-6">
        <form>
          <div className='text-gray-400 flex justify-between items-center'>
            ...
            <button type='button' className='border-0 bg-transaparent focus:outline-none'>
              <FaTimes />
            </button>
          </div>
          ...
          <div className='flex justify-between items-center bg-gray-800 rounded-xl mt-5'>
          <input type='number' min={0.01} step={0.01} className='block w-full text-sm text-slate-500 focus:outline-none cursor-pointer focus:ring-0 bg-transparent border-0 px-4 py-2' placeholder='Price (ETH)' name="price" value={price} onChange={(e) => setName(e.target.value)} required/>
          </div>
          <div className='flex justify-between items-cenetr bg-gray-800 rounded-xl mt-5'>
          <textarea type='text' className='block w-full text-sm text-slate-500 focus:outline-none cursor-pointer focus:ring-0 bg-transparent border-0 px-4 py-2' placeholder='Description' name="description" required>
            </textarea>
          </div>
